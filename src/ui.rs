...
pub fn generate_random_string_nums(amount: usize, min: f64, max: f64, max_decimals: usize, text: &mut String, random: &mut ResMut<Random>){
    ...
    for index in 0..amount{
        let mut string = format!("{:.1$}", min + random.rng.f64() * range_helper, max_decimals);

        if max_decimals != 0 {
            string = string.trim_end_matches("0").trim_end_matches(".").to_string();
            if string.is_empty(){
                string = "0".to_string();
            }
        }
                    ...
                    ui.allocate_ui(vec2(ui.available_width(), 200.0), |ui|{
                        egui::ScrollArea::vertical().auto_shrink([false, true]).show(ui, |ui|{
                            ...
                            if !is_sorting && text_edit_widget.changed(){
                                // log::info!("txt size: {} ", user_text.val.len());
